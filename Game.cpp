...
void Game::initVariables()
{
	...
	if (!Shoot.loadFromFile("Sound/shoot.wav"))
	{
	...
	if (!wink.loadFromFile("Sound/wink.wav"))
	{
		cout << "error shoot" << endl;
	}
	Wsound.setBuffer(wink);
	Wsound.setVolume(100);

	if (!music.openFromFile("Sound/bg.wav"))
	{
...
Game::Game(ScoreList* score_list)
{
	...
	this->initEnemies();
	this->initItem();
	this->font.loadFromFile("Fonts/SM.ttf");
	this->isPause = false;
	this->KeyPause = false;
	...
	this->GameOverText.setPosition(620, 100);
...
void Game::pollEvents()
{
	...
	while (this->window->pollEvent(this->ev))
	{
		...
		if (ev.Event::KeyPressed && ev.Event::key.code == Keyboard::Escape)
			this->window->close();
		if (Keyboard::isKeyPressed(Keyboard::Enter) && !KeyPause &&!Showgameover &&!Showgamewon) //กดและkeypause=true
		{
			this->KeyPause = true;
...
void Game::updateEnemies(int playerIndex)
{
	...
	for (int x = 0; x < this->enemies.size(); x++)
	{
		...
		if (enemies[x].getGlobalBounds().intersects(player[playerIndex].getGlobalBounds()))
		{
			...
			if (enemies[x].type == 1)
			{
				player[playerIndex].takeDamage(damage, pointattrack+5); //ส่งค่าคืนplayer
			}
	...
	for (size_t k = 0; k < player[playerIndex].getBullets().size(); k++)
	{
		...
		for (int j = 0; j < enemies.size(); j++)
		{
			if (player[playerIndex].getBullets()[k].getGlobalBounds().intersects(enemies[j].getGlobalBounds()))
			{
				...
				if (enemies[j].type == 1)
				{
					player[playerIndex].getPoint(pointkill+5);
				}
...
void Game::Won()
{
	...
	this->ScoreText.setPosition(760, 700);
	this->bgTexture.loadFromFile("character/end.png");
	this->bgSprite.setTexture(bgTexture);
	if (!music.openFromFile("Sound/win.wav"))
	{
		cout << "error music" << endl;
	}
	music.play();
...
void Game::GameOver()
{
	...
	this->bgSprite.setTexture(bgTexture);
	if (!music.openFromFile("Sound/lose.wav"))
	{
		cout << "error music" << endl;
	}
	music.play();
